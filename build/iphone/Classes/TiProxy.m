...
//Rare exceptions to indicate a bug in the _arkit code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_arkit = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_arkit]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.arkit.module"
